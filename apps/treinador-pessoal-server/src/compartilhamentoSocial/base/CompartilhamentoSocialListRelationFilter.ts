/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CompartilhamentoSocialWhereInput } from "./CompartilhamentoSocialWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class CompartilhamentoSocialListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => CompartilhamentoSocialWhereInput,
  })
  @ValidateNested()
  @Type(() => CompartilhamentoSocialWhereInput)
  @IsOptional()
  @Field(() => CompartilhamentoSocialWhereInput, {
    nullable: true,
  })
  every?: CompartilhamentoSocialWhereInput;

  @ApiProperty({
    required: false,
    type: () => CompartilhamentoSocialWhereInput,
  })
  @ValidateNested()
  @Type(() => CompartilhamentoSocialWhereInput)
  @IsOptional()
  @Field(() => CompartilhamentoSocialWhereInput, {
    nullable: true,
  })
  some?: CompartilhamentoSocialWhereInput;

  @ApiProperty({
    required: false,
    type: () => CompartilhamentoSocialWhereInput,
  })
  @ValidateNested()
  @Type(() => CompartilhamentoSocialWhereInput)
  @IsOptional()
  @Field(() => CompartilhamentoSocialWhereInput, {
    nullable: true,
  })
  none?: CompartilhamentoSocialWhereInput;
}
export { CompartilhamentoSocialListRelationFilter as CompartilhamentoSocialListRelationFilter };
