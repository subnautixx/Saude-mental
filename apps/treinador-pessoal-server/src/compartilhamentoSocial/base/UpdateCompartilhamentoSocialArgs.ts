/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CompartilhamentoSocialWhereUniqueInput } from "./CompartilhamentoSocialWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { CompartilhamentoSocialUpdateInput } from "./CompartilhamentoSocialUpdateInput";

@ArgsType()
class UpdateCompartilhamentoSocialArgs {
  @ApiProperty({
    required: true,
    type: () => CompartilhamentoSocialWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => CompartilhamentoSocialWhereUniqueInput)
  @Field(() => CompartilhamentoSocialWhereUniqueInput, { nullable: false })
  where!: CompartilhamentoSocialWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => CompartilhamentoSocialUpdateInput,
  })
  @ValidateNested()
  @Type(() => CompartilhamentoSocialUpdateInput)
  @Field(() => CompartilhamentoSocialUpdateInput, { nullable: false })
  data!: CompartilhamentoSocialUpdateInput;
}

export { UpdateCompartilhamentoSocialArgs as UpdateCompartilhamentoSocialArgs };
